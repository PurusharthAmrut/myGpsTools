...
public class MainActivity extends AppCompatActivity {
    ...
    public void extractAndDisplay(){        //Conditions for the toast messages need to be improved upon
        if (isExternalStorageReadable() && checkPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE)){
            switch (mTheRealDeal.messageExtract(fileName)){
                case TheRealDeal.IODP_MISMATCH_ERROR:
                    Toast.makeText(this, "First and last IODP of correction message don't match\nEXTRACTION ABORTED", Toast.LENGTH_SHORT).show();
                    break;
                case TheRealDeal.MESSAGE_INTERPRETATION_ERROR:
                    Toast.makeText(this, "Error in interpreting correction message\nEXTRACTION ABORTED", Toast.LENGTH_SHORT).show();
                    break;
                default:
                    Toast.makeText(this, "What the frac!", Toast.LENGTH_SHORT).show();
            }
            displayPrn();
            displayFastCorrections();
        }
    ...
    public void displayFastCorrections(){
        for (int i = 0; i < 210; i++){
            if (mTheRealDeal.svList[i] != null){
                infoTextView.append(Float.toString(mTheRealDeal.svList[i].fastCorrection)+"("+Integer.toString(mTheRealDeal.svList[i].udrei)+")"+", ");
            }
        }
    }
